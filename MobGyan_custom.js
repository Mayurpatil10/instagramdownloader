        ...
        $.ajax({
          ...
          success:function(data){
			...
			if(data.status=="success"){
				...
				if(data.flag=="video"){
					$('#data').html('<video id="video" width="100%" autoplay="autoplay" loop="loop" muted="muted" controls><source src="'+data.videourl+'" type="video/mp4" >Your browser does not support the video tag. </video><a href="'+data.videourl+'" class="downloadbutton pull-right" >Download</a><p style="margin-left:10px; margin-top:10px;" >Right-Click on video and save video</p>');
					$("#video")[0].load();
				}
